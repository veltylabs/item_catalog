...
# Plan — `item_catalog` trae su vista (`ui/`), sus datos de demo (`seed/`) y su demo (`web/`)

## 1. Por qué

La vista de este módulo vivía en `veltylabs/mjosefa-cms/modules/item_catalog` (la app de
...
5. **Qué borra.** `mjosefa-cms/modules/item_catalog/` completo (lo borra el plan de
...
Archivos a copiar desde `_temp/mjosefa-cms/modules/item_catalog/` — **del árbol de trabajo en
disco**: `browser.go` tiene cambios sin commitear y `specialtypicker.go` no está
trackeado; ambos son la versión buena:

| Origen | Destino | Cambios |
|---|---|---|
| `module.go` | `ui/module.go` | `package ui`. `ID = "catalog_item"` (**no** cambiar: es el recurso RBAC y la ruta de nav que ya usan las apps), `Label = "Catálogo"`. |
| `browser.go` | `ui/browser.go` | `package ui`. Conserva `requireSpecialty` (inyecta la especialidad del picker en cada `Save` y reenvía `Delete`) y las dos pestañas `decktabs`. |
| `specialtypicker.go` | `ui/specialtypicker.go` | `package ui`. |
| `svg.go` | `ui/svg.go` | `package ui`, conserva `//go:build !wasm`. |

**No** se copian: `server.go` (composition root de la app), `data/` y `docs/` (son
datos del cliente mjosefa: lista de servicios real, CSV, SQL; se quedan en la app).
...
Copiar los cuatro archivos de §3. `Browser(caller, ids, tenantID)` con la misma firma.
...
1. Especialidades: de `itemcatalog.CanonicalSpecialties`, las de slug
   `medicina-general`, `traumatologia` y `ecografia` (buscarlas por `Slug` con un
   recorrido lineal, nunca copiando sus textos). `m.UpsertSpecialty(Specialty{TenantId:
   tenantID, Prefix, Slug, Name, Position: i+1, IsPublished: true})`; guardar lo que
   devuelve (trae el `Id`).
2. Ítems, con `m.CreateItem`, `TenantId: tenantID`, `Type: itemcatalog.ItemTypeService`,
   `Currency: "CLP"` (constante local `currencyCLP`), `IsActive: true`,
   `SpecialtyId` = el `Id` devuelto en el paso 1:

   | Specialty | Sku | Name | Price |
   |---|---|---|---|
   | medicina-general | `MD001` | `Consulta Medicina General` | 25000 |
   | traumatologia | `TR001` | `Consulta Traumatología` | 35000 |
   | ecografia | `EC001` | `Ecografía Abdominal` | 40000 |

### Etapa 4 — `web/client.go`

Esqueleto de §2.4 con `itemcatalog.New(db, itemcatalog.Deps{IDs: ids, Publisher:
broker})`, `seed.Load(ic, demoTenantID)`, `loopback.WithTenant(demoTenantID, ic)`.

### Etapa 5 — tests movidos desde mjosefa-cms

Este repo tiene hoy `tests/` como **módulo Go aparte** (`tests/go.mod` + `tests/go.sum`
con `replace github.com/veltylabs/item_catalog => ../`). Eso es un defecto según su
propio `AGENTS.md` ("`tests/` is a plain directory inside the root module — never a
nested Go module: no `tests/go.mod`, no `replace`"). Cerrarlo aquí, antes de mover
tests: **borrar `tests/go.mod` y `tests/go.sum`** y correr `go mod tidy` en la raíz.
Los tests existentes deben seguir pasando sin cambios.

Luego mover (copiar) a `tests/`:

- `_temp/mjosefa-cms/tests/item_catalog_view_mock_test.go`
- `_temp/mjosefa-cms/tests/item_catalog_view_test.go`
- `_temp/mjosefa-cms/tests/item_catalog_view_wasm_test.go` (`//go:build wasm`)

En los tres: import `github.com/veltylabs/mjosefa-cms/modules/item_catalog` →
`github.com/veltylabs/item_catalog/ui`, y el selector `item_catalog.` → `ui.`. Los tests
deben pasar **sin cambiar sus aserciones**. Agregar `tests/seed_test.go`: `seed.Load` sobre `orm.New(mem.New())`
devuelve 3 especialidades y 3 ítems sin error.
...
| 2 | vista | `ui/module.go`, `ui/browser.go`, `ui/specialtypicker.go`, `ui/svg.go` |
| 3 | semilla | `seed/seed.go` |
| 4 | demo | `web/client.go` |
| 5 | tests | borrar `tests/go.mod`/`tests/go.sum`; `tests/item_catalog_view_*_test.go`, `tests/seed_test.go` |
